...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    reaction_account.parent_tweet = *ctx.accounts.tweet.to_account_info().key;
    reaction_account.reaction = reaction;

    let (_pda, bump) = Pubkey::find_program_address(
        &[
            TWEET_REACTION_SEED.as_bytes(),
            ctx.accounts.reaction_author.to_account_info().key.as_ref(),
            ctx.accounts.tweet.to_account_info().key.as_ref(),
        ],
        ctx.program_id,
    );
    reaction_account.bump = bump;
